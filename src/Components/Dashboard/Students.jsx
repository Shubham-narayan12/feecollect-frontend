...
export default function Students() {
  ...
  const fetchStudents = async () => {
    try {
      ...
      if (res.data.success) {
        setStudents(res.data.students); // 👈 backend se aaya data
      } else {
    ...
    return students.filter((s) => {
      ...
      if (search) {
        ...
        const matches =
          ...
          s.aadharNo?.toString().includes(term);
  ...
  }, [students, cls, section, session, search]);
  ...
  async function handleExcelUpload(e) {
    ...
    try {
      ...
      const res = await bulkApply(formData);
      const data = res.data;
...
Total Rows: ${data.totalRows}`,
        ...
        await fetchStudents(); // 👈 refresh list automatically
        ...
        if (data.errors && data.errors.length > 0) {
          data.errors.forEach((err, idx) => {
            toast.warn(`⚠️ ${err}`, {
              autoClose: 8000,
              toastId: `bulk-warn-${idx}`, // prevent duplicate
            });
          });
        }
      } else {
          ...
          <div className="bg-white rounded-2xl shadow-lg border-2 border-dashed border-slate-300 p-12">
            <div className="text-center">
              ...
              <p className="text-slate-500 max-w-md mx-auto">
                Apply filters or use the search bar above, then click{" "}
                      ...
                      >
                        ...
                        <td className="py-4 px-4">
                          {s.photo ? (
                            <img
                              src={s.photo}
                              className="h-14 w-14 rounded-xl object-cover border-2 border-blue-200 shadow-md"
                              alt="Student"
                            />
                          ) : (
                        ...
                        <td className="py-4 px-4">
                          <div className="flex gap-2">
                            <button
                              onClick={() =>
                                navigate(`/fee-collection?student=${s._id}`)
                              }
