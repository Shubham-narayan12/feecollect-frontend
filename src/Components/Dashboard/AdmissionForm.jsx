      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 border border-slate-200">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-3xl font-bold text-slate-800">New Admission Form</h2>
              <p className="text-slate-500 mt-1">Fill in the student details to complete admission</p>
            </div>
          </div>
        </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-6 p-3 bg-slate-50 rounded-xl border-2 border-slate-200">
                  <label className="flex items-center gap-2 cursor-pointer group">
                    <input type="radio" name="gender" value="Boy" onChange={handleInput} className="w-4 h-4 text-blue-600 cursor-pointer" />
                    <span className="text-sm font-medium text-slate-700 group-hover:text-blue-600 transition-colors">Boy</span>
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer group">
                    <input type="radio" name="gender" value="Girl" onChange={handleInput} className="w-4 h-4 text-pink-600 cursor-pointer" />
                    <span className="text-sm font-medium text-slate-700 group-hover:text-pink-600 transition-colors">Girl</span>
                  </label>
                </div>
              </div>

              <Input label="Roll Number" name="rollNo" value={form.rollNo} onChange={handleInput} />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <Input label="Father's Name" name="fatherName" value={form.fatherName} onChange={handleInput} required />
              <Input label="Mother's Name" name="motherName" value={form.motherName} onChange={handleInput} />
              <Input label="Mobile Number" name="mobile" value={form.mobile} onChange={handleInput} />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <Select label="Religion" name="religion" value={form.religion} onChange={handleInput} options={["Hindu", "Muslim", "Sikh", "Christian"]} />
              ...
              <Input label="Aadhar Number" name="aadharNo" value={form.aadharNo} onChange={handleInput} required />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <Select label="Session" name="session" value={form.session} onChange={handleInput} options={["2024-25", "2025-26", "2026-27"]} required />
              ...
              <Select label="Vehicle Type" name="vehicle" value={form.vehicle} onChange={handleInput} options={["Bus", "Van", "Auto", "---"]} />
              <Select label="Discount" name="discount" value={form.discount} onChange={handleInput} options={["No Discount", "10%", "20%", "50%"]} />

              {/* Photo Upload */}
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">Student Photo</label>
                <div className="relative">
                  <input type="file" onChange={handlePhoto} className="hidden" id="photo-upload" />
                  <label htmlFor="photo-upload" className="flex items-center justify-center w-full px-4 py-3 border-2 border-dashed border-slate-300 rounded-xl cursor-pointer hover:border-blue-500 hover:bg-blue-50 transition-all duration-200">
                    {form.photo ? (
                      <img src={form.photo} className="h-24 w-24 rounded-xl object-cover shadow-md" alt="Student" />
                    ) : (
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <DocRadio label="Transfer Certificate (TC)" name="tc" value={form.tc} onChange={handleInput} />
              <DocRadio label="Character Certificate" name="charCert" value={form.charCert} onChange={handleInput} />
              <DocRadio label="Report Card" name="reportCard" value={form.reportCard} onChange={handleInput} />
              <DocRadio label="Date of Birth Certificate" name="dobCert" value={form.dobCert} onChange={handleInput} />
            </div>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <label className="flex items-center gap-2 cursor-pointer group">
            ...
            />
            <span className="text-sm font-medium text-slate-700 group-hover:text-emerald-600 transition-colors">Yes</span>
          </label>
          <label className="flex items-center gap-2 cursor-pointer group">
            <input
              type="radio"
            ...
            />
            <span className="text-sm font-medium text-slate-700 group-hover:text-red-600 transition-colors">No</span>
          </label>
        </div>
      </div>
    </div>
