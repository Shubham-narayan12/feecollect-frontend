...
// ─── Gallery Data ───────────────────────────────────────────────────────────
const galleryCategories = ["All", "Events", "Classroom", "Sports", "Activities"];
...
function Lightbox({ image, onClose, onPrev, onNext }) {
  ...
  return (
    <div
      ...
      style={{ background: "rgba(5, 10, 20, 0.96)", backdropFilter: "blur(20px)" }}
      onClick={onClose}
    >
      {/* Close */}
      <button
        onClick={onClose}
        className="absolute top-6 right-6 z-10 w-11 h-11 rounded-full flex items-center justify-center text-white transition-all duration-200"
      ...
      <button
        ...
        className="absolute left-4 md:left-8 z-10 w-12 h-12 rounded-full flex items-center justify-center text-white transition-all duration-200 hover:scale-110"
        style={{ background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.15)" }}
      >
        <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
          <path d="M13 3L6 10l7 7" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
        </svg>
      </button>

      {/* Image */}
      <div
        className="relative mx-20 md:mx-32 max-w-4xl w-full"
        style={{ maxHeight: "85vh" }}
        onClick={(e) => e.stopPropagation()}
      >
        <div className="relative overflow-hidden shadow-2xl" style={{ aspectRatio: "16/10", border: "3px solid #7EC870" }}>
          <img
            src={image.src}
            alt={image.title}
          ...
          >
            <span
              className="inline-block text-xs font-semibold tracking-widest uppercase mb-2 px-3 py-1"
              style={{ background: "rgba(126,200,112,0.25)", color: "#7EC870", letterSpacing: "0.12em", border: "1px solid #7EC870" }}
            >
              {image.category}
            </span>
      ...
      <button
        ...
        className="absolute right-4 md:right-8 z-10 w-12 h-12 rounded-full flex items-center justify-center text-white transition-all duration-200 hover:scale-110"
        style={{ background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.15)" }}
      >
        <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
          <path d="M7 3l7 7-7 7" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
        </svg>
      </button>
    </div>
  );
...
function GalleryCard({ image, index, onClick }) {
  ...
  const aspectClass = image.tall ? "aspect-[3/4]" : image.wide ? "aspect-[16/9]" : "aspect-square";
  ...
  return (
    ...
    >
      <div className={`relative w-full ${image.tall ? "h-full min-h-[320px]" : aspectClass}`} style={{ overflow: "hidden" }}>
        <img
          src={image.src}
          alt={image.title}
          loading="lazy"
        ...
        >
          ...
          >
            ...
            <p className="text-blue-200 text-xs mt-1 font-medium tracking-wide">View Full Image →</p>
          </div>
        </div>
      </div>
    </div>
...
export default function GallerySection() {
  ...
  const currentIdx = lightboxImage ? lightboxImages.findIndex((i) => i.id === lightboxImage.id) : -1;
  ...
  const goNext = useCallback(() => {
    if (currentIdx < lightboxImages.length - 1) setLightboxImage(lightboxImages[currentIdx + 1]);
  ...
  return (
    <>
      ...
      <section
        className="bg-white"
        style={{ padding: "60px 0 72px" }}
      >
        <div style={{ maxWidth: "1280px", margin: "0 auto", padding: "0 24px" }}>

          {/* Header — matches screenshot: left-aligned, dark green title, yellow underline */}
          <div
            className="mb-10"
            style={{
              opacity: mounted ? 1 : 0,
          ...
          >
            ...
            <div style={{ width: "60px", height: "4px", background: "#e6a817", borderRadius: "2px" }} />
          </div>
          ...
          {hasMore && (
            <div className="flex justify-center mt-12">
              <button
                ...
                className="group flex items-center gap-2.5 px-8 py-3 font-semibold text-sm transition-all duration-300"
                style={{
                  background: "white",
                  color: "#1a4731",
                ...
                onMouseEnter={(e) => {
                  ...
                  e.currentTarget.style.boxShadow = "0 6px 20px rgba(26,71,49,0.25)";
                }}
                onMouseLeave={(e) => {
          ...
          <p
            ...
            style={{ color: "#a0aec4", fontWeight: 500, letterSpacing: "0.05em" }}
          >
