...
export const MonthlyFeesSection = ({
  ...
  if (!FeeLedger) return <div>Loading Monthly Fees...</div>;

  const [hasTransport, setHasTransport] = useState(false);

  // ✅ Paid months identify karo
  const paidMonths =
    FeeLedger?.monthlyRecords
      ?.filter((m) => (m.paidAmount || 0) > 0 && (m.dueAmount || 0) === 0)
      ?.map((m) => m.month) || [];

  // ✅ UI me sirf unpaid months dikhao
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              Select months and enter fee amounts
            </p>
          </div>
        </div>
              ...
              <div className="flex items-center gap-2 bg-white rounded-xl p-3 shadow-sm">
                ...
                <p className="text-sm text-gray-700">
                  <span className="font-bold text-blue-600">
                    Auto-fill for Class {student?.className}:
                  </span>
                  {defaultTuitionFee > 0 && (
      ...
      <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl p-6 border-2 border-gray-200 shadow-inner">
        <div className="space-y-3">
          {filteredMonthsList.map((m) => (
            ...
            >
              <div
                className={`${hasTransport ? "col-span-3" : "col-span-3"} flex items-center gap-3`}
              >
                <input
                  type="checkbox"
                ...
                />
                <span
                  className={`font-bold ${monthlyFees[m] ? "text-blue-700" : "text-gray-600"}`}
                >
                  {m}
                </span>
              </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-gray-700">
                {Object.keys(monthlyFees).filter((m) => monthlyFees[m]).length}{" "}
                month(s) selected
              </span>
            </div>
